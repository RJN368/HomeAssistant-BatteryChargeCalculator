...
def _install_stub_sensors_module() -> type:
    ...
    return AxelRemoteControlSensor
...
def _import_axel_remote_control_sensor_class():
    """Import only the Axel sensor submodule without loading sensors/__init__."""
    package_name = "custom_components.battery_charge_calculator.sensors"
    module_name = f"{package_name}.axel_remote_control"
    ...
    return module.AxelRemoteControlSensor


@pytest.mark.asyncio
async def test_axel_sensor_not_registered_when_axel_disabled() -> None:
    AxelRemoteControlSensor = _install_stub_sensors_module()
    ...
    assert not any(isinstance(entity, AxelRemoteControlSensor) for entity in entities)


@pytest.mark.asyncio
async def test_axel_sensor_registered_when_axel_enabled() -> None:
    AxelRemoteControlSensor = _install_stub_sensors_module()
    sensor_module = _import_sensor_module()
    ...
    assert any(isinstance(entity, AxelRemoteControlSensor) for entity in entities)


def test_axel_remote_control_diagnostics_schema_unavailable() -> None:
    AxelRemoteControlSensor = _import_axel_remote_control_sensor_class()

    coordinator = MagicMock()
    coordinator._axel_cache = {
    ...
    coordinator.config_entry.options = {
        const.AXEL_FAIL_SAFE_MODE: const.AXEL_FAIL_SAFE_MODE_OPEN,
        const.AXEL_NEUTRALIZE_ON_ACTIVE_ENTRY: True,
        const.AXEL_POLL_INTERVAL_SECONDS: 60,
        const.AXEL_REQUEST_TIMEOUT_SECONDS: 10,
    }
    coordinator._axel_overlapping_window.return_value = None
    coordinator._axel_cache_age_seconds.return_value = None

    sensor = AxelRemoteControlSensor(MagicMock(), coordinator)
    ...
    assert attrs["source_status"] == const.AXEL_SOURCE_STATUS_UNAVAILABLE


def test_axel_remote_control_diagnostics_active_window_fields() -> None:
    AxelRemoteControlSensor = _import_axel_remote_control_sensor_class()

    now_utc = datetime.now(timezone.utc)
    active_window = AxelWindow(
    ...
    coordinator = MagicMock()
    coordinator._axel_cache = {
        "source_status": const.AXEL_SOURCE_STATUS_STALE,
        "is_active": True,
        "suppression_reason": const.AXEL_SUPPRESSION_REASON_ACTIVE_WINDOW,
        "last_transition_reason": const.AXEL_TRANSITION_REASON_ACTIVE_ENTRY,
        "last_error": None,
        "last_success_utc": now_utc,
    }
    coordinator.config_entry.options = {
        const.AXEL_FAIL_SAFE_MODE: const.AXEL_FAIL_SAFE_MODE_CLOSED,
        const.AXEL_NEUTRALIZE_ON_ACTIVE_ENTRY: True,
        const.AXEL_POLL_INTERVAL_SECONDS: 120,
        const.AXEL_REQUEST_TIMEOUT_SECONDS: 15,
    }
    coordinator._axel_overlapping_window.return_value = active_window
    coordinator._axel_cache_age_seconds.return_value = 120.0

    sensor = AxelRemoteControlSensor(MagicMock(), coordinator)

    assert sensor._attr_native_value == "active"
    attrs = sensor._attr_extra_state_attributes
    assert attrs["source_status"] == const.AXEL_SOURCE_STATUS_STALE
    assert attrs["suppression_reason"] == const.AXEL_SUPPRESSION_REASON_ACTIVE_WINDOW
    assert attrs["last_transition_reason"] == const.AXEL_TRANSITION_REASON_ACTIVE_ENTRY
    ...
    assert attrs["fail_safe_mode"] == const.AXEL_FAIL_SAFE_MODE_CLOSED
