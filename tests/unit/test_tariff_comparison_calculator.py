...
class TestBasicImportOnlyCost:
    def test_two_slots_fixed_rate_import_cost(self):
        ...
        assert result["monthly"][0]["import_cost_gbp"] == pytest.approx(0.20, abs=0.01)
    ...
    def test_no_export_earnings_when_export_slots_none(self):
        ...
        assert result["monthly"][0]["export_earnings_gbp"] == 0.0
    ...
    def test_net_cost_equals_import_minus_export(self):
        ...
        assert result["monthly"][0]["net_cost_gbp"] == pytest.approx(
            result["monthly"][0]["import_cost_gbp"], abs=0.01
        )
...
class TestImportAndExport:
    def test_net_calculation_import_export(self):
        ...
        assert result["monthly"][0]["import_cost_gbp"] == pytest.approx(
            expected_import, abs=0.01
        )
        assert result["monthly"][0]["export_earnings_gbp"] == pytest.approx(
...
class TestMonthlyAggregation:
    ...
    def test_monthly_rows_have_required_keys(self):
        """Each monthly row must expose import, export, SC, and net_cost keys."""
        import_slots, rate_map = self._build_inputs()
        result = calculate_tariff_cost(
            import_slots=import_slots,
            import_rate_map=rate_map,
        ...
        for month_row in result["monthly"]:
            assert required_keys <= month_row.keys(), (
                f"Missing keys in monthly row {month_row!r}"
            )
