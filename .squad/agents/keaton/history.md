...
### 2026-05-27 — Axle VPP spec implementation-readiness review

**Lead clarifications applied to `_docs/axle-vpp-awareness-spec.md`:**

- Confirmed architectural boundary: Axle awareness gates command dispatch in `BatteryChargeCoordinator._async_update_data`; planning (`octopus_state_change_listener`) remains unchanged in MVP.
- Confirmed tariff comparison isolation: `TariffComparisonCoordinator` must remain unaffected by Axle enablement and fallback behavior.
...
- Added explicit acceptance criteria for non-Axle regression safety and `simulate_only` invariants.
- Added rollout safety controls: opt-in default, reversible via options toggle, and constrained MVP blast radius.
- Left product decisions open for confirmation: Axle API payload contract, neutralization default, and fail-safe default confirmation.

### 2026-05-28 — Axle VPP implementation plan finalized

**Implementation planning outcomes for `_docs/axle-vpp-awareness-spec.md`:**
...
- Added a file-by-file implementation map covering new Axle client/window modules, config flow/schema insertion, diagnostics sensor wiring, and required test modules.
- Codified contract-preservation checks for `simulate_only`, non-Axle behavior parity, and tariff-comparison isolation.
