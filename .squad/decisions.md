...
## Active Decisions (Axel VPP Awareness)
*Added: 2026-05-27 — sources: Dallas (feature spec), Keaton (lead review)*

### D-AX-1: MVP architecture is command-path gating only

**Decision:** Axel awareness is applied only to charge/export command dispatch in `BatteryChargeCoordinator._async_update_data` for MVP. Planning and forecast generation remain active and unchanged.
...
**Decision:** Axel awareness is opt-in and disabled by default. `simulate_only` behavior is preserved in all Axel states, and non-Axel behavior is a hard regression guard.

**Fallback decision:** Fail-open after prolonged Axel API unavailability is the default for MVP, with optional fail-closed mode.

*Agreed by: Dallas, Keaton — 2026-05-27*

### D-AX-3: Tariff comparison stays independent

**Decision:** Axel suppression/fallback state must not block `TariffComparisonCoordinator` refreshes or tariff sensor behavior.

**Why:** Tariff comparison is explicitly non-impacted by the Axel rollout.

*Agreed by: Keaton — 2026-05-27*

### D-AX-4: Config flow placement and interval semantics

**Decision:** Axel settings are inserted after `ml_settings` and before tariff comparison steps in both initial and options flows.
...
**Decision:** Include a dedicated diagnostic sensor for Axel remote-control status in MVP.
...
### Open questions — Axel VPP awareness (pending product-owner confirmation)

| # | Question | Default | Blocking? |
|---|---|---|---|
| OQ-AX-1 | Axel API contract details and authentication scheme | Await confirmation | No |
| OQ-AX-2 | Entering active Axel window: issue one-time neutralization command or suppress-only | Suppress-only for MVP | No |
