"""Options flow for battery charge calculator."""
...
async def _fetch_available_tariffs(
    ...
    """Fetch available Octopus import tariff products and convert to selector options."""
...
def _tariff_codes_from_stored_json(tariffs_json: str) -> list[str]:
    """Extract import tariff codes from the stored JSON string."""
    if not tariffs_json:
        return []
    try:
...
def _tariff_codes_to_stored_json(
    ...
    """Convert selected tariff codes to the coordinator's JSON format."""
...
class BatteryChargCalculatorFlowHandler(config_entries.OptionsFlow):
    ...
    def _with_provider_defaults(self, values: dict) -> dict:
        """Return a copy with all planning provider selectors populated."""
        merged = dict(values)
        merged.setdefault(
    ...
    async def async_step_provider_selection(self, user_input=None):
        """Step 1b – choose concrete providers used by planning abstractions."""
        if user_input is not None:
            self.options.update(
                {
                    const.PLANNING_TARIFF_PROVIDER: user_input.get(
                        const.PLANNING_TARIFF_PROVIDER,
                        const.DEFAULT_PLANNING_TARIFF_PROVIDER,
                    ),
                    const.PLANNING_BATTERY_PROVIDER: user_input.get(
                        const.PLANNING_BATTERY_PROVIDER,
                        const.DEFAULT_PLANNING_BATTERY_PROVIDER,
                    ),
                    const.PLANNING_SOLAR_PROVIDER: user_input.get(
                        const.PLANNING_SOLAR_PROVIDER,
                        const.DEFAULT_PLANNING_SOLAR_PROVIDER,
                    ),
                    const.PLANNING_AXLE_PROVIDER: user_input.get(
                        const.PLANNING_AXLE_PROVIDER,
                        const.DEFAULT_PLANNING_AXLE_PROVIDER,
                    ),
                    const.PLANNING_TEMPERATURE_PROVIDER: user_input.get(
                        const.PLANNING_TEMPERATURE_PROVIDER,
                        const.DEFAULT_PLANNING_TEMPERATURE_PROVIDER,
                    ),
                }
        ...
        self.options = self._with_provider_defaults(self.options)
        return self.async_show_form(
            step_id="provider_selection",
            data_schema=_planning_providers_schema(
                tariff_provider=self.options.get(
                    const.PLANNING_TARIFF_PROVIDER,
                    const.DEFAULT_PLANNING_TARIFF_PROVIDER,
                ),
                battery_provider=self.options.get(
                    const.PLANNING_BATTERY_PROVIDER,
                    const.DEFAULT_PLANNING_BATTERY_PROVIDER,
                ),
                solar_provider=self.options.get(
                    const.PLANNING_SOLAR_PROVIDER,
                    const.DEFAULT_PLANNING_SOLAR_PROVIDER,
                ),
                axle_provider=self.options.get(
                    const.PLANNING_AXLE_PROVIDER,
                    const.DEFAULT_PLANNING_AXLE_PROVIDER,
                ),
                temperature_provider=self.options.get(
                    const.PLANNING_TEMPERATURE_PROVIDER,
                    const.DEFAULT_PLANNING_TEMPERATURE_PROVIDER,
                ),
            ),
    ...
    def _save_and_exit(self):
        return self.async_create_entry(
            title=const.TITLE,
            data=self._with_axle_defaults(self._with_provider_defaults(self.options)),
        )
