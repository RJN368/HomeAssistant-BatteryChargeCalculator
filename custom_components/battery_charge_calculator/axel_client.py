...
class AxelEvent:
    """Normalized event payload returned by Axel endpoint."""
...
class AxelClientError(RuntimeError):
    """Raised when Axel event retrieval fails after retries."""


class AxelClient:
    """Thin async client for the Axel home-assistant event endpoint."""

    def __init__(
        self,
        api_token: str,
        *,
        endpoint: str = const.AXEL_EVENT_ENDPOINT,
        request_timeout_seconds: int = const.DEFAULT_AXEL_REQUEST_TIMEOUT_SECONDS,
        max_retries: int = const.AXEL_MAX_RETRIES,
        retry_base_delay_seconds: float = const.AXEL_RETRY_BASE_DELAY_SECONDS,
        retry_jitter_seconds: float = const.AXEL_RETRY_JITTER_SECONDS,
    ...
    async def async_fetch_event(
        ...
        """Fetch the current/upcoming Axel event.

        Returns None for no-event payloads (empty/null/missing start_time).
        """
        headers = {
            "Authorization": f"Bearer {self._api_token}",
            "Accept": const.AXEL_EVENT_ACCEPT_HEADER,
        }
        ...
        for attempt in range(total_attempts):
            try:
                async with session.get(
                    ...
                    return _normalize_axel_event(payload)
                ...
                if not retryable or is_last_attempt:
                    ...
                    raise AxelClientError(
                        f"Failed to fetch Axel event after {attempt + 1} attempt(s): {sanitized}"
                    ) from err

                delay_seconds = self._retry_backoff_delay(attempt)
                _LOGGER.debug(
                    "Axel event fetch failed (attempt %s/%s): %s; retrying in %.2fs",
                    attempt + 1,
                    total_attempts,
...
def _normalize_axel_event(payload: Any) -> AxelEvent | None:
    """Normalize raw payload to AxelEvent or None for no-event responses."""
    if payload in (None, "", [], {}):
        return None
