...
class BatteryChargCalculatorConfigFlow(config_entries.ConfigFlow, domain=const.DOMAIN):
    ...
    async def async_step_provider_selection(self, user_input=None):
        ...
        self._heating_data = self._with_provider_defaults(self._heating_data)
        return self.async_show_form(
            step_id="provider_selection",
            data_schema=_planning_providers_schema(
                tariff_provider=self._heating_data.get(
                    const.PLANNING_TARIFF_PROVIDER,
                    const.DEFAULT_PLANNING_TARIFF_PROVIDER,
                ),
                battery_provider=self._heating_data.get(
                    const.PLANNING_BATTERY_PROVIDER,
                    const.DEFAULT_PLANNING_BATTERY_PROVIDER,
                ),
                solar_provider=self._heating_data.get(
                    const.PLANNING_SOLAR_PROVIDER,
                    const.DEFAULT_PLANNING_SOLAR_PROVIDER,
                ),
                axle_provider=self._heating_data.get(
                    const.PLANNING_AXLE_PROVIDER,
                    const.DEFAULT_PLANNING_AXLE_PROVIDER,
                ),
                temperature_provider=self._heating_data.get(
                    const.PLANNING_TEMPERATURE_PROVIDER,
                    const.DEFAULT_PLANNING_TEMPERATURE_PROVIDER,
                ),
            ),
    ...
    def _create_entry(self):
        ...
        options = self._with_provider_defaults({**self._main_data, **self._heating_data})
        options = self._with_axle_defaults(options)
...
from .config_flow_options import BatteryChargCalculatorFlowHandler
